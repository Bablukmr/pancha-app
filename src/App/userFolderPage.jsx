...
export default function UserFolderPage() {
  ...
  const [loadingUserFolder, setLoadingUserFolder] = useState(false);
  ...
  useEffect(() => {
    if (token && userId) getUserFolder();
  }, [token, userId]);

  const getUserFolder = () => {
    setLoadingUserFolder(true);
    axios
      .get(`https://api.pancha.kids/pancha/user-folder?id=${userId}`, {
        // .get("https://api.pancha.kids/pancha/user-folder", {
        headers: {
  ...
  const handleAddFolder = (e) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setNotificationBody("New folder created.");
        setNotificationType("success");
        shownotiftion();
      })
      .catch((err) => {
    ...
    <>
      ...
      <div className="w-full  flex flex-col items-center justify-center">
        ...
        {loadingUserFolder ? (
          ...
          userFolder.length === 0 && (
            <div className="w-[80%] text-center h-[100px] mt-10">
              <p>No User Folder found !!</p>
            </div>
          )
        )}
            ...
            <ul className="w-full  my-2 max-h-[160px] overflow-auto flex flex-col  rounded-md p-1 items-start ">
              {userFolder.map((keyword) => (
                ...
                >
                  ...
                  <li key={keyword.id} className="w-full h-full">
                    {keyword.name}
                  </li>
                </div>
              ))}
